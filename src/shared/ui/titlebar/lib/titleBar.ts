...
const MINIFY_TO_TRAY_NOTIFIED_KEY = 'minToTrayNotified';
...
export async function windowClose() {
  try {
    ...
    const minToTrayNotified = localStorage.getItem(MINIFY_TO_TRAY_NOTIFIED_KEY) ?? 'false';
    ...
    localStorage.setItem(MINIFY_TO_TRAY_NOTIFIED_KEY, 'true');
  } catch (error) {
    // eslint-disable-next-line no-console
